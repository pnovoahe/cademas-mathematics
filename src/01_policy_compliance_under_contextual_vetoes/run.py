...
Aligned with the companion paper, Section 5.2 (Policy Compliance under Contextual Vetoes).
"""
...
def run_monte_carlo(
    ...
    write_json(
        meta_path,
        {
            ...
            "paper": "Aggregation Semantics for Prioritization in Cooperative Automated Decision Making",
...
def write_snippets(
    ...
    captions = [
        ...
        "",
        "## Figure: policy_compliance_operator_agreement",
        "",
        (
            "Pairwise agreement among seven ranking configurations "
            "($A_L$ and $A_G$ at representative $\\lambda$, plus $A_M$). "
            "(\\textbf{a})~Kendall $\\tau$ on the union of the two Top-$K$ index sets. "
            "(\\textbf{b})~Jaccard similarity between the corresponding Top-$K$ sets."
        ),
        "",
        r"Suggested LaTeX label: \label{fig:policy-operator-agreement}",
        "",
    ]
...
All defaults live in `src/common/config.py`. Group sizes are derived
...
cd src/01_policy_compliance_under_contextual_vetoes
python run.py
...
`companion paper` Section 5.2. Do not invent numerical claims.
Do not describe any operator as globally superior.

- Code: `src/01_policy_compliance_under_contextual_vetoes/run.py`
- Config: `src/common/config.py` (`veto_fraction={settings.veto_fraction:.2f}`)
