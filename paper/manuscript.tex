...
We do not propose new aggregation operators. We adopt three well-established families from fuzzy connectives and MCDM that encode distinct compensation semantics \citep{Zimmermann1980,Klement1996}. Each choice of $A$ defines a different policy reading for the CADEMAS integration layer:
\begin{equation}
...
\begin{enumerate}
\item \textit{Conjunctive (non-compensatory) regime.} Classical fuzzy set intersection is modeled by operators such as the minimum and the algebraic product. In both cases, a low score on one dimension cannot be offset by a high score on the other \citep{Zimmermann1980,Klement1996}:
\begin{equation}
...
\item \textit{Disjunctive (permissive) regime.} Fuzzy union is typically represented by the maximum, which allows either dimension to dominate the aggregated score \citep{Zimmermann1980,Klement1996}:
\begin{equation}
A_S(R_i, Q_i) = \max(R_i, Q_i).
\end{equation}
This regime emphasizes the most favorable signal and suits exploratory settings where high risk or strong contextual alignment alone may justify attention. Prioritized OR-type operators in MCDM follow the same permissive logic when one criterion may override the others \citep{Li2019}.

\item \textit{Non-linear compensatory regime.} Intermediate behavior between full compensation and strict non-compensation is often modeled by the weighted geometric mean \citep{Zimmermann1980,Turksen1992,Krejci2018,Mohammadi2023,Natoli2024}:
\begin{equation}
...
Table~\ref{tab:sim_protocol} summarizes the protocol by aspect; the subsections below retain the full specification.
...
\end{itemize}
The disjunctive maximum ($A_S^{\max}$) is defined in Section~\ref{subsect:nonlinear_aggregators} for completeness but is excluded from all simulations. CADEMAS instantiations in this paper target governance settings in which contextual non-compliance should limit prioritization; under $A_S^{\max}$, a vetoed case with $Q_i = 0$ still receives $P_i = R_i$, so predictive risk alone can place institutionally excluded alternatives in the Top-$K$ tier. That behavior is the opposite of the compliance and firewall questions we stress-test, and it would collapse the contrast between compensatory and veto-preserving operators. The noise experiment compares how contextual perturbations propagate under linear versus conjunctive integration; $A_S^{\max}$ is $\lambda$-independent and always tracks $\max(R_i, Q_i)$, offering little additional insight on that axis. We therefore reserve $A_S^{\max}$ for theoretical discussion (Proposition~\ref{prop:rank_reversal}) rather than empirical benchmarking.
...
We set $\lambda = 0.5$ (balanced risk--context trade-off, as in \citet{Godz2026}) and compare full-cohort rankings under $A_L$, $A_T^{\min}$, and $A_G$. We do not include $A_S^{\max}$ here for the same governance reasons as in the simulations: employee retention prioritization in CADEMAS-ML is not an exploratory setting where either high risk or high contextual alignment alone should dominate (e.g., a low-risk employee with strong contextual fit would rank near the top under $A_S^{\max}$, which is misaligned with a risk-aware retention policy). The three operators retained span the compensatory, veto-preserving, and intermediate regimes most relevant to this workflow. Figure~\ref{fig:real_example}a locates each employee in the $(Q_i, R_i)$ plane; Figure~\ref{fig:real_example}b reports the Top-$K$ prioritization tier with $K = 2$ (10\% of the cohort), mimicking a resource-constrained retention programme.
