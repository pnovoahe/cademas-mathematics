...
\Author{Pavel Novoa-HernĂˇndez$^{1,*}$\orcidA{}, Mariia Godz$^{2}$\orcidA{} and David A. Pelta$^{2}$\orcidC{}}

%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Pavel Novoa-HernĂˇndez, Mariia Godz and David A. Pelta}
...
The empirical part of the paper has two layers. First, we run controlled Monte Carlo simulations to examine three aspects of operator behavior: policy compliance under increasing predictive weight, resilience to localized model overconfidence, and rank stability under noisy fuzzy context. Second, we apply the same operators to a curated 50-case subsample from a CADEMAS-ML employee-attrition cohort and select a Top-$K$ tier with $K = 5$ (10\% of the subsample). The case study does not replace the simulations; it shows how the same semantics appear in a concrete prioritization task when $\lambda$ shifts toward context or risk, and when a context-vetoed employee can enter the intervention tier under risk-dominated linear integration.
...
\item We validate the interpretation on a curated 50-case employee-attrition subsample from CADEMAS-ML, comparing $A_L$ at $\lambda \in \{0.1, 0.5, 0.9\}$ with $A_T^{\min}$ and $A_G$, and showing how operator choice and $\lambda$ jointly reshape the Top-$K$ tier.
\end{itemize}
...
The simulations above deliberately use controlled populations so that each failure mode can be observed in isolation. We now ask whether the same operator semantics remain visible in a concrete CADEMAS workflow at a smaller but still resource-constrained scale: a curated subsample of $n = 50$ employees from the IBM HR Analytics attrition cohort and a Top-$K$ tier with $K = 5$ (10\% of the subsample). Scores are drawn from the CADEMAS-ML instantiation described by \citet{Godz2026}. The subsample and the parent 100-case pool are available under \texttt{experiments/data/attrition50/} and \texttt{experiments/data/attrition100/}\footnote{Files \texttt{cases\_attrition\_50.csv}, \texttt{cases\_attrition\_50\_final.csv}, and the corresponding 100-case CSVs.}.

The subsample contains 50 employees with balanced attrition labels (25 confirmed leavers, 25 stayers) drawn from Research \& Development (30) and Sales (20). Cooperative risk $R_i$ and contextual alignment $Q_i$ (Digital / Technological Transformation scenario) are taken from the precomputed CADEMAS-ML scores; on this subsample, $R_i \in [0.001, 0.989]$ and $Q_i \in [0, 0.73]$, with 12 employees at $Q_i = 0$. The attrition label is retained for verification only and does not enter $P_i$.

We compare five integration settings: $A_L$ at $\lambda = 0.1$ (context-favoring), $\lambda = 0.5$ (balanced baseline, as in \citet{Godz2026}), and $\lambda = 0.9$ (risk-dominated, mirroring the high-$\lambda$ regime in Section~\ref{subsect:opportunity_cost}), together with $A_T^{\min}$ and $A_G$ at $\lambda = 0.5$. We do not include $A_S^{\max}$ here for the same governance reasons as in the simulations. Figure~\ref{fig:real_example_plane} locates all 50 employees in the $(Q_i, R_i)$ plane; Figure~\ref{fig:real_example_bump} traces rank trajectories across the five integration settings for the Top-$K$ tier selected under baseline $A_L$ ($\lambda = 0.5$, $K = 5$).

\begin{figure}[t]
\centering   
\includegraphics[width=0.7\textwidth]{figures/real_example_plane.pdf}
\caption{Real attrition example: cooperative risk vs.\ context score ($n = 50$, Digital Transformation context). Grey points are the full subsample; colored circles identify the baseline Top-$K$ tier under $A_L$ ($\lambda = 0.5$), and gray stars the outsiders who enter the tier under a non-baseline integration setting.}
...
\caption{Rank trajectories across $A_L$ at $\lambda = 0.1$, $0.5$, and $0.9$, plus $A_T^{\min}$ and $A_G$ (both at $\lambda = 0.5$), for the Top-$K$ tier under baseline $A_L$ ($\lambda = 0.5$; $n = 50$, $K = 5$). Gray star markers on non-baseline columns indicate employees outside the baseline Top-$K$ who enter the intervention tier under that setting; labels report their rank under $A_L$ ($\lambda = 0.5$) and name. Each colored trajectory uses a distinct hue; employee names appear at the first column only.}
\label{fig:real_example_bump}
\end{figure}

Table~\ref{tab:real_example} lists the Top-$K$ tier under $A_L$ at $\lambda = 0.5$ and reports scores and ranks under all five settings. At the balanced baseline, $A_L$ ($\lambda = 0.5$), $A_T^{\min}$, and $A_G$ agree on the Top-$K$ head: \textit{Laura Baker}, \textit{Andrew Wood}, \textit{Alice Brown}, \textit{Andrew Ross}, and \textit{Caroline Bennett}, each combining elevated $R_i$ with moderate-to-strong contextual alignment. This consensus reflects the regime where jointly high risk and context dominate and operator choice is secondary at the very top.

The low-$\lambda$ linear setting inverts that logic. Under $A_L$ with $\lambda = 0.1$, \textit{Julia Smith}---who attains the maximum contextual score in the subsample ($Q_i = 0.73$) but minimal cooperative risk ($R_i = 0.005$)---ranks first, ahead of \textit{Laura Baker} and \textit{Andrew Wood}. Under baseline $A_L$ ($\lambda = 0.5$), the same employee falls to rank~25; under $A_T^{\min}$ and $A_G$, $P_i$ collapses toward zero (ranks~35). Context-favoring linear integration therefore elevates a low-risk, high-context profile that non-linear operators keep far from the intervention tier.

The high-$\lambda$ linear setting exposes the compliance failure observed in simulation. Under $A_L$ with $\lambda = 0.9$, \textit{Camila Phillips}---the highest-risk employee ($R_i = 0.99$) but context-vetoed ($Q_i = 0$)---rises from rank~18 (baseline $A_L$) to rank~5 and enters the Top-$K$ tier. Under $A_T^{\min}$ and $A_G$, she remains at $P_i = 0$ (rank~39). No employee with $Q_i = 0$ appears in the Top-$K$ tier under baseline $A_L$ ($\lambda = 0.5$) or under the non-linear operators; the violation is specific to the risk-dominated linear configuration. \textit{Eleanor Foster} illustrates intermediate sensitivity: rank~7 under baseline $A_L$ but rank~2 when $\lambda = 0.9$, as predictive weight reshuffles the upper tier without a veto.

These patterns complement the simulation results. Sweeping $\lambda$ on the same observed scores shows context-dominated, balanced, and risk-dominated linear regimes side by side, while $A_T^{\min}$ and $A_G$ enforce a zero-context boundary regardless of $\lambda$. The bump chart in Figure~\ref{fig:real_example_bump} makes the joint effect of operator choice and $\lambda$ visible for the same five employees selected under $A_L$ ($\lambda = 0.5$).

\begin{table}[t]
\centering
\scriptsize
\caption{Top-$K$ prioritization under baseline $A_L$ ($\lambda = 0.5$) for the 50-case attrition subsample (Digital Transformation context, $K = 5$). $P_i^{(L-)}$, $r^{(L-)}$: $A_L$ at $\lambda = 0.1$; $P_i^{(L)}$, $r^{(L)}$: $A_L$ at $\lambda = 0.5$; $P_i^{(L+)}$, $r^{(L+)}$: $A_L$ at $\lambda = 0.9$; remaining columns: $A_T^{\min}$ and $A_G$ at $\lambda = 0.5$. Ranks are over all $n = 50$ cases (1 = highest $P_i$).}
\label{tab:real_example}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lrrrrrrrrrrrrr}
\toprule
\textbf{Case} & $R_i$ & $Q_i$ & $P_i^{(L-)}$ & $P_i^{(L)}$ & $P_i^{(L+)}$ & $P_i^{(\min)}$ & $P_i^{(G)}$ & $r^{(L-)}$ & $r^{(L)}$ & $r^{(L+)}$ & $r^{(\min)}$ & $r^{(G)}$ & \textbf{Attr.} \\
\midrule
Laura Baker & 0.9359 & 0.6000 & 0.6336 & 0.7679 & 0.9023 & 0.6000 & 0.7493 & 2 & 1 & 1 & 1 & 1 & Yes \\
Andrew Wood & 0.9288 & 0.5667 & 0.6029 & 0.7477 & 0.8925 & 0.5667 & 0.7255 & 3 & 2 & 4 & 2 & 2 & Yes \\
Alice Brown & 0.9295 & 0.5600 & 0.5969 & 0.7447 & 0.8925 & 0.5600 & 0.7215 & 4 & 3 & 3 & 3 & 3 & Yes \\
Andrew Ross & 0.9302 & 0.4667 & 0.5130 & 0.6984 & 0.8839 & 0.4667 & 0.6589 & 7 & 4 & 11 & 4 & 4 & Yes \\
Caroline Bennett & 0.9368 & 0.4571 & 0.5051 & 0.6970 & 0.8888 & 0.4571 & 0.6544 & 8 & 5 & 9 & 5 & 5 & Yes \\
\bottomrule
\end{tabular}
\end{table}
